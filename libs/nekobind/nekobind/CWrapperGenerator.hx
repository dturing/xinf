...
//			print( "}\nDEFINE_PRIM_MULT(bind_"+settings.className+"_"+f.name+");\n\n" );
	...
	public function handleClass( e:TypeInfos, c:Classdef ) {
		...
		if( settings.cStruct != null ) {
			...
			// destructor lock
			/*
			if( settings.dtorLock!=null ) {
				print("typedef struct _clock _clock;\n");
				print("_clock *context_lock_new();\n");
				print("void context_lock( _clock *l );\n");
				print("void context_release( _clock *l );\n");
				print("void context_lock_delete( _clock *l );\n");
				
				print("_clock *"+cStruct+"_dtorLock = NULL;\n\n");
				
				print("void "+cStruct+"_dtorLock_init() {\n");
				print("\t"+cStruct+"_dtorLock = context_lock_new();\n");
				print("}\nDEFINE_ENTRY_POINT("+cStruct+"_dtorLock_init);\n\n");
			}
			*/
			...
			if( settings.dtor != null ) {
				...
				if( settings.dtorLock != null )
					print("\tcontext_lock( "+cStruct+"_dtorLock );\n");

				print("\t"+settings.prefix+translator.translate(settings.dtor)+"( self );\n" );

				if( settings.dtorLock != null )
					print("\tcontext_release( "+cStruct+"_dtorLock );\n");
